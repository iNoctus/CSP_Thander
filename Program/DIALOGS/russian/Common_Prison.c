	...
	switch(Dialog.CurrentNode)
	{
		...
		case "CheckGun":
			if (GetQuestPastDayParam("pchar.questTemp.PrisonGun") < 30)
			{
				...
				Link.l1 = "Вот черт! Так закрутил"+ GetSexPhrase("ся","ась") +", что потерял"+ GetSexPhrase("","а") +" счёт времени... Извините, приду позже, как и договаривались.";
				Link.l1.go = "exit";
				NextDiag.TempNode = "First_officer";
				break;
			}
        ...
        case "First_prisoner":
			...
			{
				dialog.text = LinkRandPhrase("Меня взяли за грабёж, дело шьют...", "Агрх, холен"+ GetSexPhrase("ый","ая") +" ты... Тебя бы ко мне в камеру на недельку! Кхе-хе-хе...", "Я ни в чем не виноват!");
        ...
        case "ToPrisonHead_canMove_1":
            dialog.text = "Ну, так пойдём отсюда быстрее! Черт возьми, как я рад!";
		...
		case "Node_4":
			...
			link.l1 = "Та-ак, похоже, сейчас у тебя будут серьёзные проблемы, приятель. Но все-таки я хочу послушать твои объяснения.";
			link.l1.go = "Node_4_1";
		break;
