void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "SeekSpy_2":
			...
			Link.l1 = "Может, и ни при чем, но давай-ка, для начала, пройдём в комендатуру. Там разберемся...";
	  ...
	  // to_do
		...
		break;
		case "SitySpy_4":
            dialog.text = "Черт! Надо же было так глупо вляпаться!.. Все ясно, буду следовать за тобой - мне моя жизнь ещё дорога...";
		...
		case "DestroyPirate_4":
			...
			if (pchar.GenQuest.DestroyPirate.fortPlace == "1")
			{
				...
				dialog.text = "Хм, дальше... Я слишком близко подошёл к форту, поэтому не буду рисковать и придержу тебя на время здесь. Своим ты скажешь, что пойдёшь на моем корабле до места, под названием " + sTemp + ", а они пусть ждут тебя в " + XI_ConvertString("Colony"+sld.city+"Dat") +
			...
			else
			{
				...
				lngFileID = LanguageOpenFile("LocLables.txt");
				sTemp = LanguageConvertString(lngFileID, pchar.GenQuest.DestroyPirate.outShoreId);
				dialog.text = "А дальше ты скажешь своим, что пойдёшь на моем корабле до места, под названием " + sTemp + ", а они пусть ждут тебя в " + XI_ConvertString("Colony"+sld.city+"Dat") +
					". В этом случае я оставлю тебе жизнь. Иначе тебе вспорят брюхо прямо здесь, на палубе.\nКак перспективы на будущее?";
