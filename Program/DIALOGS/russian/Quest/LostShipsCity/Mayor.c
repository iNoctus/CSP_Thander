...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "FT_7_1":
			dialog.text = "Пойдём дальше. Второе: запрещается строить корабли, плоты и прочие плавсредства. Выбраться отсюда все равно невозможно, а разрушать Город нельзя.";
		...
		case "DestrNarval_BadOk":
			dialog.text = "М-да, неприятно несколько... Ну да ладно, все-таки это была резня, а не лёгкая прогулка по палубе. В общем, я рад, что все кончилось хорошо. Хочу подкинуть тебе деньжат за работу. Десять тысяч золотых!";
