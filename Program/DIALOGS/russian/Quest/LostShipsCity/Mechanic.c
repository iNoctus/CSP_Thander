...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		break;
		case "FT_25":
			dialog.text = "А-а, ну это возможность спускаться под воду в моем костюме. Дело в том, что под водой завелись здоровенные твари, которые ходят по дну и жрут все, что попало. Вот я и не рискую теперь спускаться. Я ведь учёный, а не боец...";
			link.l1 = "Понятно. Так что я долж"+ GetSexPhrase("ен","на") +" сделать?";
			link.l1.go = "FT_26";
		break;
		case "FT_26":
			dialog.text = "Сначала вы должны купить у меня клиппер. Затем достать со дна кое-что, без чего мы не сможем освободить фрегат от стискивающих его кораблей.";
		...
		break;
		case "FT_33":
			dialog.text = "Но он же все равно узнает, если ещё не узнал. Потому, хорошо бы было уведомить его от лица виновника торжества, так сказать. С Чадом нужно вести себя очень аккуратно, иначе...";
			link.l1 = "Что иначе?";
			link.l1.go = "FT_34";
		break;
		case "FT_34":
		...
		break;
		case "FreeCorvett_1":
			dialog.text = "Я уже все подготовил. У меня на платформе стоит лебёдка, к ней я прикрепил обломки вокруг фрегата. Так что стоит только привести механизм в действие - и вуаля, корабль свободен!";
		...
		case "FreeCorvett_7":
			...
			AddSimpleRumourCityTip("А я не верю, что этот Механик так важен для Города. Сколько жили без него, проживём и еще...", "LostShipsCity", 10, 1, "LSC", "");		
		...
		case "DeepAgain":
			...
			link.l1 = "Опять деньги нужны? Я же принёс"+ GetSexPhrase("","ла") +" вам полтора миллиона!";
		...
		//Набор команды
		case "TakeCrew":
			dialog.text = "Прекрасно, "+ GetSexPhrase("друг мой","дорогая моя") +", все уже размещено в трюме вашего клиппера. Теперь вам нужно набрать команду из жителей Города. Сами понимаете, вдвоём мы с фрегатом не управимся.";
		...
		case "inPrison_7":
			...
			link.l1 = "Я понял"+ GetSexPhrase("","а") +", спасибо вам... Может быть, все-таки попытаться открыть это чёртову клетку?";
