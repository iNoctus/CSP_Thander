	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				link.l1 = "Спасибо за тёплый приём, хотя моё прибытие оказалось несколько подмоченным!.. Меня зовут капитан " + GetFullName(pchar) + ".";
				link.l1.go = "FT_1";
			}
			else
			{
		...
		break;
		case "FT_6":
			dialog.text = "Вспоминать будете потом, сначала дослушайте... Так вот, Чад Каппер - глава Города. Должность эта выборная, так что он пришёл к власти, так сказать, демократическим путём. В его обязанности входит следить за исполнением Закона...";
		...
		break;
		case "FT_8":
			dialog.text = "Ну, может и так... Продолжу. К адмиралу на регистрацию нужно явиться как можно быстрей. Сейчас шторм, поэтому народу на палубах маловато. Но как только он закончится - Каппер сам вас найдёт в любом случае. И я советую не тянуть с регистрацией.";
			link.l1 = "Хорошо, это я понял"+ GetSexPhrase("","а") +". И когда, по вашему мнению, закончится этот шторм?";
			link.l1.go = "FT_9";
		break;
		case "FT_9":
			dialog.text = "Завтра к вечеру точно все успокоится. Шторм уже пошёл на убыль.";
			link.l1 = "Ясно. Что ещё мне нужно знать?";
			link.l1.go = "FT_10";
		break;
		case "FT_10":
			dialog.text = "Адмирал расскажет вам о Законе, по которому мы все живём, более подробно, но я могу в двух словах описать его постулаты.";
		...
		break;
		case "FT_12":
			dialog.text = "Конечно, нет! Это запрещено Законом!.. Вы знаете, я бы не хотел углубляться в эти темы, это не моё дело. Идите к адмиралу, он вам все расскажет.";
			link.l1 = "И где он находится?";
			link.l1.go = "FT_13";
		break;
		case "FT_13":
			dialog.text = "На варшипе 'Сан Августин'. Отсюда это довольно далеко, к варшипу ведёт мост... Извините, но точней объяснить я не могу, так как вы не знаете ориентиров. Ищите 'Сан Августин'!";
		...
		case "FT_17":
			...
			link.l1 = "Хм, ну что же, понятно. Буду иметь в виду. Спасибо ещё раз за тёплый прием.";
			link.l1.go = "FT_18";
		break;
		case "FT_18":
		...
		case "SeekCitizen_Choice_1":
			...
			else
			{
				...
				if (sld.sex == "man")
				{
					...
					Link.l2 = "Нет, не о нем. Давайте ещё раз назову.";
			...
			else
			{
				...
				else
				{
					if (sld.location == "LostShipsCity_town")
					{
						...
						{
							dialog.text = LinkRandPhrase("Она здесь, на этом корабле. Ищите внимательней.", "Ха, так она здесь, на этом корабле!", "Она на этом корабле, странно, что вы ещё не увидели ее.");
						}
						else
						{
		...
		break;
		case "ansewer_2":
			dialog.text = "Нет, около трёх месяцев. Был ночной бой с пиратами, которые захватили огромный военный корабль. Наш капитан был из разряда отчаянных безумцев, а потому и вступил в заведомо проигрышное сражение на бриге. Разумеется, первым же бортовым залпом линкор смёл все живое и не очень с нашей палубы, включая капитана и половину офицеров.\n"+
				"Ничего подобного мне переживать ещё не приходилось, я бросился на палубу и едва смог пробраться туда сквозь обломки рангоута и клочья снастей... Это зрелище ни с чем не сравнимо, капитан. Хотя, вы, наверное, знаете, что уж я\n"+
		...
		break;
		case "ansewer_2_1":
			dialog.text = "Ещё бы! Так дух захватило, что я чуть в штаны не наложил... Но все уже позади, а мне тут и вовсе нравится. Спокойно, тихо, интересно.";
		...
		break;
		case "ansewer_3":
			dialog.text = "Более чем. Но ещё больше прибывает в последнее время.";
		...
		break;
		case "ansewer_4_1":
			dialog.text = "Полузатонувший корабль, он треснул пополам и держится каким-то чудом на рифе. Там никто не живёт - слишком опасно.";
		...
		break;
		case "ansewer_4_3":
			dialog.text = "И таверна, и церковь, и магазин - все как у людей в обычном городе! Есть ещё ростовщик по имени Августо Брамс, но с ним стоит вести себя поосторожней...";
		...
		break;
		case "ansewer_6":
			dialog.text = "Сначала будет сложно, но вы быстро разберётесь, что к чему.";
			link.l1 = "М-да, обнадёживающее заявление... Ну, а карта Города имеется?";
			link.l1.go = "ansewer_6_1";
			NextDiag.(NodePrevName).l6 = true;
		break;
		case "ansewer_6_1":
			dialog.text = "Имеется, я даже видел одну такую.";
			link.l1 = "Вот! И где вы её видели?";
		...
		case "SeekCrew_2":
			...
			link.l1 = "Отлично! Тогда немедленно отправляйтесь на 'Сан Габриэль'. Хенрик Ведекер переправит вас на моё судно.";
