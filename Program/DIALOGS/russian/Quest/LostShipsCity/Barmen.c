	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			else
			{
				...
				{
					link.l4 = "Послушай, я так и не наш"+ GetSexPhrase("ел","ла") +" тебя в этом твоём винном погребе...";
					link.l4.go = "TizerCellarLate";
				}
				if (pchar.questTemp.LSC == "toDiffindoor" && CheckCharacterItem(pchar, "letter_LSC"))
				{
					link.l4 = "Я обследовал"+ GetSexPhrase("","а") +" 'Диффиндур' и наш"+ GetSexPhrase("ел","ла") +" там сундук Тизера. В нем, кроме прочего, было ещё и очень интересное письмо...";
				...
				{
					link.l4 = "Слушай, я не смог"+ GetSexPhrase("","ла") +" к полуночи быть в твоём погребе. Так что я никого не видел"+ GetSexPhrase("","а") +" и нечего не услышал"+ GetSexPhrase("","а") +".";
					link.l4.go = "InterceptionLate";
				}
				if (pchar.questTemp.LSC == "InterceptionYouSeen")
				{
					link.l4 = "Ты знаешь, ровно в полночь я был"+ GetSexPhrase("","а") +" в твоём погребе. Адмирал и ещё какой-то тип собирались поговорить, но заметили меня и ушли. Так что я ничего не услышал"+ GetSexPhrase("","а") +".";
		...
		break;
		case "FS_2":
			dialog.text = "Это ничего не значит, когда речь идёт о водах вокруг Города! Просто шторм пошёл на убыль, и именно это дало тебе шанс.";
			link.l1 = "Понятно...";
			link.l1.go = "exit";
		break;
		...
		break;
		case "ansewer_5":
			dialog.text = "Это ещё ничего! Бывало и похуже. Штормы здесь - обычное дело.";
			link.l1 = "Понятно...";
			link.l1.go = "ansewer_5_1";
			NextDiag.(NodePrevName).l5 = true;
		break;
		case "ansewer_5_1":
			dialog.text = "Этот шторм закончится завтра ближе к вечеру. У нас положено по прибытии отмечаться у адмирала, так что если до окончания шторма ты не явишься к нему, то он найдёт тебя сам.";
			link.l1 = "Адмирал?";
		...
		break;
		case "ansewer_2":
			dialog.text = "Не знаю, это тебя надо спросить, как ты сюда заплыл"+ GetSexPhrase("","а") +". Ты же капитан, своим кораблём сам"+ GetSexPhrase("","а") +" и управлял"+ GetSexPhrase("","а") +". Это я ничего не знаю, сидел себе в каюте, вдруг крики, потом треск обшивки... Вот и все.";
		...
		break;
		case "ansewer_3":
			dialog.text = "Ха!!! И откуда ты так"+ GetSexPhrase("ой","ая") +" умн"+ GetSexPhrase("ый","ая") +" взял"+ GetSexPhrase("ся","ась") +"?.. Были попытки, но все неудачные. Получается, что люди берут материалы и попросту портят их. От этого Город серьёзно страдает, так как это понижает его устойчивость. Если высвободить какой-нибудь корабль, то остров начинает двигаться, все переходы ломаются, каюты встают по другому\nВ общем, это боком выходит. Поэтому сейчас это запрещено нашим Законом. И уже давно!";
			link.l1 = "Понятно...";
			link.l1.go = "int_quests";
			NextDiag.(NodePrevName).l3 = true;
		break;
		case "ansewer_4":
			dialog.text = "Да вполне нормально. Хотя, знаешь, надоело все до смерти... А вообще, Город живёт на основании Закона. О его главных положениях тебе лучше узнать у адмирала Каппера.";
		...
		break;
		case "ansewer_6_4":
			dialog.text = "Знаешь что, "+ GetSexPhrase("приятель","" + pchar.name + "") +", вот что я тебе скажу: никому не болтай об этом! Если адмирал узнает, что кто-то нарушил Закон, да ещё в этом и преуспел... В общем, этому вестнику не поздоровится.";
			link.l1 = "Хм, не понимаю...";
			link.l1.go = "ansewer_6_5";
		break;
		case "ansewer_6_5":
			dialog.text = "Поймёшь потом. А пока, ради твоего же блага, делай так, как я говорю. Не упоминай имя Тизера! Понял"+ GetSexPhrase("","а") +"?";
		...
		break;
		case "ansewer_7_1":
			dialog.text = "Флейт этот по идее должен был пойти ко дну - он буквально расколот пополам. Однако, как-то держится на поверхности, зацепился, видать, за рифы. В общем, корабль этот особняком стоит, никто там не живёт и жить не будет.";
			link.l1 = "Почему?";
		...
		break;
		case "ansewer_9_1":
			dialog.text = "Ну, что тебе сказать? Все сундуки заперты на замки трёх уровней сложности. Если простой ключ можно найти даже на улице, то с остальными куда сложней. Ведь именно в сундуки с хорошими замками люди прячут действительно ценные вещи.\n"+
				"Даже если ты раздобудешь ключи, имей в виду, что такие ключи есть и у других. Если хочешь что-то спрятать надёжно, то лучше искать сундуки подальше, за Городом.";
		...
		break;
		case "ansewer_10_1":
			dialog.text = "Хм, даже как он сюда попал - никто не знает, а ты хочешь, чтобы я тебе что-то объяснил. Он даёт деньги в долг, но лучше от него держаться подальше"+ GetSexPhrase(", приятель","") +".";
		...
		break;
		case "Tizer_1":
			dialog.text = "Ну, погребом я называю его по привычке, а вообще это одна из кают здесь. Нужно выйти на основную палубу 'Флерона' и с неё зайти в квартердек.";
		...
		case "TizerCellarLate":
			dialog.text = "Это мне известно, ведь я ждал тебя там битый час! Черт возьми, нельзя же быть так"+ GetSexPhrase("им","ой") +" легкомысленн"+ GetSexPhrase("ым","ой") +" в таком серьёзном деле!";
			link.l1 = "В каком таком серьёзном?";
			link.l1.go = "TizerCellarLate_1";
		break;
		case "TizerCellarLate_1":
			dialog.text = "Ещё раз говорю, что я не могу об этом деле болтать с тобой прилюдно! Это безумие!";
			link.l1 = "Ладно, не кипятись. Давай ещё раз попробуем встретиться, раз это так необходимо.";
		...
		break;
		case "TizerCellar_2":
			dialog.text = "Ты, наверное, уже в курсе того, что Закон Города запрещает строить что-либо, на чем можно отсюда уплыть. Так вот, я просто ума не приложу, как это Тизер смог построить втихаря некое корыто, да ещё и успешно на нем добраться до Большой земли.";
		...
		case "TizerCellar_7":
			...
			{
				link.l1 = "Хм, ты знаешь, на 'Диффиндуре' я уже побывал"+ GetSexPhrase("","а") +". И наш"+ GetSexPhrase("ел","ла") +" там сундук Тизера. В нем, кроме прочего, было ещё и очень интересное письмо...";
		...
		break;
		case "TizerCellar_8":
			dialog.text = "Интересно, очень интересно... Однако, это опять же ничего не доказывает. Каппер выслушает тебя, а затем убьёт на глазах остальных граждан как возмутител"+ GetSexPhrase("я","ьницу") +" спокойствия. И никто ему и слова скажет. Ключ? Да ключей в Городе полно, практически каждый сундук закрыт на ключ.";
		...
		break;
		case "TizerCellar_10":
			dialog.text = "Точно! Мы на верном пути!\nОтправляйся к 'Диффиндуру'. Полагаю, что там ты найдёшь предмет, к которому данный ключ и подойдёт. Тизер, хоть и казался мне человеком с причудами, тем не менее был далеко не дурак, судя по всему...";
		...
		case "TizerCellarOk_2":
			dialog.text = "Так-так, очень интересно... Он пишет: '...так как мы долго и очень серьёзно готовились.' Значит, он был не один...";
		...
		case "TizerCellarOk_4":
			...
			link.l1 = "Почему? Как отменим запрет на постройку кораблей, так сразу и займёмся этим. Там и решим, что да как строить.";
		...
		case "TizerCellarOk_6":
			dialog.text = "И ещё нужно не забывать, что адмирал вряд ли будет стоять, слушать тебя и репу чесать в растерянности. Требовать отмены положения Закона, который внесён Каппером и ревностно им исполняется, - большой риск. Мы должны убедить адмирала в своей правоте, а сделать это будет непросто, как мне кажется.";
		...
		case "TizerCellarOk_8":
			dialog.text = "В общем, все это мои домыслы, к ним мы вернёмся позднее. А сейчас найди Андрэ Лабора и аккуратно выясни все о его участии в деле Тизера.";
		...
		case "AfterProtector_1":
			...
			link.l1 = "Хм... Ну, в общем, поговорил"+ GetSexPhrase("","а") +" я с плотником, этим Андре Лабором, и он попросил меня пройти в каюту корвета 'Протектор'. Я туда заш"+ GetSexPhrase("ел","ла") +", ну, и жду его, ничего такого не думаю даже... А этот Андрэ привёл милиционера на разговор со мной! Пришлось всех отправить на тот свет.";
			link.l1.go = "AfterProtector_2";
		break;
		case "AfterProtector_2":
			dialog.text = "Дьявол! Это надо же! Ну, теперь тебе конец... Слушай, будь так добр"+ GetSexPhrase("","а") +", не рассказывай, что это я тебя надоумил. Тебя уже все равно убьют, а я ещё пожить могу...";
			link.l1 = "Веди себя как мужчина! Никто меня не убьёт, ведь никто и не знает, что это сделал"+ GetSexPhrase("","а") +" именно я. Они не успели рассказать обо мне адмиралу.";
		...
		case "AfterProtector_7":
			dialog.text = "У всех членов клана 'Нарвал' есть амулет с изображением перевёрнутой пятиконечной звезды. Достань амулет и иди с ним к адмиралу. Расскажешь ему о том, что наш"+ GetSexPhrase("ел","ла") +" его на месте убийства милиционера и плотника.";
		...
		case "AfterNarvalDesrt_2":
			dialog.text = "Н-да, лихо... Ну, "+ GetSexPhrase("братец","сестрёнка") +", медаль тебе положена. Хотя, если как следует поразмыслить, вреда от твоего деяния больше, чем пользы.";
			link.l1 = "Как понять?";
		...
		break;
		case "Interception_2":
			dialog.text = "Так вот, будь там сегодня до полуночи, встань возле решётки за колонной и подожди. Только повернись так, чтобы плечи твои не было видно из-за колонны. Возможно, тебе удастся подслушать очень интересный разговор, который повлияет на твоё решение о дальнейших действиях.";
			link.l1 = "Чей разговор?";
			link.l1.go = "Interception_3";
		break;
		case "Interception_3":
			dialog.text = "Адмирала и ещё одного человека. Мне самому давно уже интересно, о чем это они там периодически беседуют... В общем, сделай это, и, полагаю, обстановка во многом прояснится.";
		...
		case "InterceptionBadWork_2":
			dialog.text = "Как-то я случайно столкнулся с ними у решётки погреба, когда они разговаривали. Начал наблюдать за ними и понял, что они втихаря встречаются здесь время от времени. Хотел бы я услышать, о чем они говорят...";
			link.l1 = "Ну, может ещё не поздно? Давай подождём их следующую встречу.";
			link.l1.go = "InterceptionBadWork_3";
		break;
		case "InterceptionBadWork_3":
			dialog.text = "Подождём, конечно, но только я сам все сделаю теперь. Интересно было бы услышать их разговор именно сейчас, после устроенной тобой резни 'нарвалов'. А теперь надо будет подождать...";
		...
		case "Result":
			dialog.text = NPCStringReactionRepeat("Пока нет, но я стараюсь. Жди.",
                ...
                "Не надоело ещё?", "block", 1, npchar, Dialog.CurrentNode);
		...
		case "InterceptionOk_5":
			...
			link.l1 = "Вот дьявол!.. Послушай, ты же давно здесь живёшь, должен знать всех.";
		...
		break;
		case "NB_2":
			dialog.text = "Его нашли мёртвым в винном погребе таверны. Знаешь эту каюту на корме, где стоят винные бочки? Вот это там его и нашли...";
			link.l1 = "Вот это да! Кого-нибудь видели? Убийцу нашли?";
			link.l1.go = "NB_3";
		break;
		case "NB_3":
			dialog.text = "Нет, никто ничего не видел. Армо, наша официантка, нашла его уже мёртвым. Она пошла в погреб, когда Хилл задержался там надолго, и вот...";
		...
		case "ADied_1":
			dialog.text = "Куда там! Никто даже не видел, как это произошло. Она пошла прибираться в носовую часть таверны, там, где стоит кровать. Вдруг мы услышали её крик. И все...";
		...
		break;
		case "ADied_3":
			dialog.text = "Значит, они все-таки вышли из-под контроля. М-да... А за что они её убили?";
		...
		break;
		case "SeekCrew_2":
			dialog.text = "Да уж... Знаешь, я, пожалуй, откажусь. Подожду, когда на это будет разрешение, и соберётся адмирал.";
			link.l1 = "Ты многим рискуешь, оставаясь. На Город идёт сильный шторм.";
