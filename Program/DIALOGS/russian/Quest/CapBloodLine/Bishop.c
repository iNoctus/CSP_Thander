...
/*
    ...
    */
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
            ...
            if (npchar.id == "Bridgetown_Mayor" )
            {
                ...
                if( Pchar.questTemp.CapBloodLine.stat == "PrepareToEscape3" )
                {
            		...
            		link.l1 = "Добрый день, губернатор. Я принёс ещё профилактического экстракта для вашей супруги и хотел бы иметь возможность осмотреть ее.";
            		link.l1.go = "SStep_9";
            		break;
                }
		...
		case "CGStep_2":
        	dialog.text = "Ах, так я ещё должен заботиться о здоровье мятежников! Черт побери! Вы думаете, что мы будем его лечить? Вдоль всей дороги от Вестона до Бриджуотера расставлены виселицы, и он подойдёт для любой из них. Полковник Кирк научит этих дураков-протестантов кое-чему такому, о чем будут помнить их дети, внуки и правнуки!";
		...
		case "SStep_8":
            ...
            dialog.text = "Это большая сумма, доктор. Я выдам её, но прошу отчитаться по возвращении - я все проверю.";
            link.l1 = "Я должен поспешить.";
            link.l1.go = "Exit";
            NextDiag.TempNode = "SStep_4";
        ...
        case "WStep_12":
        	...
        	link.l1 = "Это будет затруднительно, но выбора у меня нет. Что насчёт шлюпа? Никто из рабов не сможет просто прийти на верфь и прикупить себе шлюп, как ни в чем не бывало.";
            link.l1.go = "WStep_13";
...
//            SetCharacterGoods(Pchar,GOOD_RUM,40);//600);
        ...
        case "DKStep_6":
        	...
        	link.l1 = "Хорошо, я учту это. А теперь иди готовься, мы все ещё не на свободе.";
            link.l1.go = "Exit_RunAway";
            Pchar.questTemp.CapBloodLine.sLocator = "reloadShip";
            Pchar.questTemp.CapBloodLine.iTime = 3;
