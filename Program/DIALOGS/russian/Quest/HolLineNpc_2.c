...
//>>>>>>>>>>>>========= Разброс диалогов по персонажам =====================
            if (pchar.questTemp.State == "DelivLettTortuga_NotFoundLetter")// кэп пиратов, вывезший де Барраса из Тортуги
            {
    			...
    			link.l1 = "Я хочу знать, где этот чёртов Антонио?! Отвечай!!!";
    			link.l1.go = "Step_H7_1";
            }
            if (npchar.id == "LoranDeGraf")// Лоран де Граф, квест №8
            {
    			dialog.text = "Судар"+ GetSexPhrase("ь","ыня") +", сейчас не время для разговоров! Идёт бой.";
...
//********************************* Диалоги кэпа, который увёз де Барраса из Тортуги **********************************
 		case "Step_H7_1":
			dialog.text = "Антонио де Баррас? Я высадил его по дороге сюда на испанский военный галеон.";
			link.l1 = "Дьявол! Как назывался этот галеон и куда он ушёл?";
			link.l1.go = "Step_H7_2";
		break;
  		case "Step_H7_2":
			dialog.text = "Галеон 'Изабелла', ушёл курсом на Мейн к Санта-Каталине. Антонио говорил мне, что ему обязательно и срочно нужно именно туда.";
			link.l1 = "А-а-а, черт! Слушай, ты же пират, какого дьявола ты якшаешься с испанцами?!";
			link.l1.go = "Step_H7_3";
		break;
  		case "Step_H7_3":
			dialog.text = "Не твоё собачье дело, каррамба...\n"+
    	...
    	break;
		case "Step_H8_11":
			dialog.text = "А-а-а, вот как? Хорошо, это можно устроить. Книгу я продать не смог и, собственно, спрятал её в надёжном месте. Предлагаю такие условия сделки: вы платите мне 235 тысяч, и я даю вам карту того места, где лежит книга.";
			link.l1 = "А не слишком ли много - 235 тысяч?";
			link.l1.go = "Step_H8_12";
    	break;
		case "Step_H8_12":
			dialog.text = "Нет. Дело в том, что в том схроне ещё кое-что лежит. Так что это цена за все, за весь клад.";
            if (sti(pchar.money) >= 235000)
            {
            ...
            {
                link.l1 = "Нет таких денег у меня сейчас. Придётся заглянуть к тебе позже.";
		...
		case "Step_H8_14":
			...
			link.l1 = "Это точно. Я благодар"+ GetSexPhrase("ен","на") +" вам, Лоран де Граф. Надеюсь, ещё увидимся.";
			link.l1.go = "exit";
