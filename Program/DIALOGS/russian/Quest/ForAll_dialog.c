...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "PKInMarigo_6":
			...
			link.l1 = "Полагаю, что этим шансом я пользоваться не буду. Я хочу знать все ещё и о предметах роскоши, падения цен на которые вы так опасаетесь...";
		...
		break;
		case "LSCQuestMent_2":
			dialog.text = "Андре Лабор – законопослушный гражданин города. А вот тебе придётся все мне рассказать о Тизере Дэне, иначе завтра тобой будет заниматься адмирал.";
		...
		break;
		case "NewDeckSharp_1":
			dialog.text = "Да все путём, "+ GetSexPhrase("приятель","дорогуша") +", жаловаться грех\nНу, рассказывай, зачем пожаловал"+ GetSexPhrase("","а") +" на этот раз?";
		...
		case "DeckSharp_over":
			dialog.text = "Мы все обсудили, "+ GetSexPhrase("приятель","подруга") +". На твоём месте я бы не стал терять времени.";
		...
		case "PortmansCap_3":
			...
			else
			{
				if (sti(npchar.quest.stepsQty) < 5)
				{
					...
					dialog.text = "Хм, ты знаешь, я уже завёл новый журнал. Тем не менее, старый журнал мне нужен ещё, необходимо все перенести в новый. Так что я могу забрать его за " + FindRussianMoneyString(sti(npchar.quest.money)) + ". и немного моих личных драгоценностей.";
		...
		case "PortmansCap_inDeck_2":
			...
			else
			{
				...
				{
					npchar.quest.money = makeint(sti(npchar.quest.money) / sti(npchar.quest.stepsQty)); //уменьшаем вознаграждение
					dialog.text = "Черт, так, значит, вот где я его потерял!.. Спасибо тебе, конечно, но дело в том, что ты долго меня искал"+ GetSexPhrase("","а") +", и я уже завёл новый. Однако, перенести кое-что мне оттуда нужно, поэтому я заберу журнал у тебя. Но много заплатить не могу...";
					link.l1 = "А сколько можешь?";
					link.l1.go = "PortmansCap_inDeck_3";
				}
				else
				{
					dialog.text = "Черт, так, значит, вот где я его потерял!.. Спасибо тебе, конечно, но дело в том, что ты долго меня искал"+ GetSexPhrase("","а") +", и я уже завёл новый. Старые записи перенёс по памяти, так что мне он не нужен.";
		...
		break;
		case "SeekCap_2":
			dialog.text = "Дьявол! Ну, для меня ещё не все потеряно. Я все-таки попытаюсь отправить тебя на тот свет...";
		...
		case "SCQ_RapeWife_1":
			...
			dialog.text = "Я небогат, но с радостью отдам все, что у меня есть! Если вы освободите мою жену и доставите её ко мне, то я заплачу вам " + FindRussianMoneyString(sti(npchar.quest.money)) + " и отдам все свои личные драгоценности.";
		...
		case "SCQ_manResult":
			...
			switch (npchar.quest.SeekCap)
			{
				...
				case "manFriendover":
					...
					link.l1 = "Наш"+ GetSexPhrase("ел","ла") +" и рассказал"+ GetSexPhrase("","а") +", что вы его ждёте здесь. Так что все вышло удачно.";
		...
		case "SCQ_Revenge_3":
			...
			link.l1 = "Хм... "+ GetSexPhrase("а","да, мужчины на это способны. Но,") +" может, вы остынете? Ну, пройдёт день-другой, и все изменится...";
		...
		case "FriendCap_inDeck_2":
			dialog.text = "О-о-о, нашёлся все-таки! Очень хорошо... Вы меня извините за грубый приём, я, знаете ли, грешным делом подумал, что вы пират.";
		...
		case "OffM_Ok":
			...
			dialog.text = "Хорошо. Но это ещё не все. Я требую единовременного пособия, так сказать. Поиздержался без работы, вот и надо оплатить долги. Думаю, что " + FindRussianMoneyString(sti(Npchar.quest.OfficerPrice.add)) + " меня устроят.";
			link.l1 = "Чего?!!";
