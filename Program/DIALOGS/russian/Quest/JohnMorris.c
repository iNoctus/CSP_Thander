	...
	switch(Dialog.CurrentNode)
	{
        ...
        case "I_know_you_good":
			dialog.text = LinkRandPhrase(GetFullName(pchar) + ", рад тебя видеть! Зачем пожаловал"+ GetSexPhrase("","а") +" на этот раз?",
                                         ...
                                         "Так-так, "+ GetFullName(pchar)+ " у меня в гостях. Хотел бы я ещё раз увидеть, как ты управляешься с клинком... Ну что, какие вопросы, "+ GetSexPhrase("друг","подруга") +"?");
		...
		break;
		case "SharpPearl_2":
			dialog.text = "Понятия не имею. Меня жемчужные дела не интересуют. Хотя, на твоём месте я бы поискал его в борделях. Рано или поздно ты его там застанешь...";
		...
		break;
 		...
 		case "Step_3":
			dialog.text = "Генерал-губернатор Ямайки относится ко мне, мягко говоря, не дружелюбно. Поэтому его приглашение поучаствовать в деле вызывает у меня серьёзные опасения. Что-то здесь не так.";
 		...
 		case "Step_5":
            ...
            {
    			dialog.text = "Я видел на рейде четыре корабля. Если ещё и я пойду с вами, то при штурме мы будем только мешать друг другу. Так что без меня на этот раз.";
	 	...
	 	case "Gold_found_in_fort":    // Нода разговора в форте Куманы
    		...
    		{
            	dialog.text = "Проклятье, мой галеон пошёл ко дну! Я потерял корабль и добрую четверть команды. Жаль хороших парней..."+
        ...
        break;
	 	...
	 	case "Step_7":
			...
			link.l1 = "Всего хорошего, Джон. Ещё встретимся!";
			link.l1.go = "Step_8";
        break;
        ...
        break;
        case "Step_F8_3":
            dialog.text = "Хех, мне не нравится идея нападать на голландцев и я, пожалуй, соглашусь на твоё предложение. Но тебе придётся кое-что сделать для меня.";
            link.l1 = "Что именно?";
        ...
        break;
        case "Step_F8_8":
            dialog.text = "Так-так... Да, я предполагал, что этот павлин во всех красках распишет свои подвиги. Ну что ж, эту тетрадку я переправлю с надёжными людьми Мэдифорду, и уверен, больше он меня шантажировать не будет. А ты можешь успокоить голландцев - их я пальцем не трону!";
		...
		//********************* Пиратская линейка *********************
		case "PL_Q8_deck":
			dialog.text = "И что ты делаешь у меня на палубе? Ведь идём на Порто Белло!";
			link.l1 = "Да, я помню...";
			link.l1.go = "exit";
