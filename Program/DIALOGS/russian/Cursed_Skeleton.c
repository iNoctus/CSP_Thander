...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				scareOfficers(50);
				dialog.text = "Как ты смеешь нарушать покой в моей гробнице, смертн"+ GetSexPhrase("ый","ая")+"? Да ещё и роешься в моих вещах! Стоит преподать тебе урок.";
				npchar.quest.meeting = "1";
				link.l1 = "Вот черт, я думал" +GetSexPhrase("","а")+" что здесь никто не живёт. Прошу прощения, я сейчас же уйду.";
				link.l1.go = "meeting_1";
				//Если мы пришли по церковному квесту
				if(CheckAttribute(cursedLocLoad, "DestroyGhost"))
				{
					link.l1 = "Я при" + GetSexPhrase("шёл","шла") + " избавить это место от скверны! Изыди, нечистый! Именем господа, я заклинаю...";
			...
			else if (npchar.quest.meeting == "2")
			{
				...
				link.l1 = "Дьявол, как ты здесь оказался? Постой, я ещё смогу найти  твой череп!";
				...
				if(CheckCharacterItem(pchar, pchar.questTemp.Cursed.Item) && CheckAttribute(pchar,"cursed.Skullbought"))
				{
					...
					link.l2 = "Ещё чего. Теперь череп мой. Спасибо за наводку, без тебя я бы его не наш"+GetSexPhrase("ел","ла")+".";
		...
		case "question":
			dialog.text = "Я давно думал об этом. С черепом я смогу вернуть себе прошлую жизнь, своё тело. Может быть даже раздобуду корабль и верну свою дурную славу. Нет, умирать я не точно собираюсь.";
		...
		case "end_quest_battle":
			...
			if(CheckCharacterItem(pchar, pchar.questTemp.Cursed.Item) && CheckAttribute(pchar,"cursed.Skullbought"))
				{
					dialog.text = "Зря ты это затеял" + GetSexPhrase("","а") + ", ещё никому не удалось одолеть меня в такой форме.";
		...
		case "meeting_2":
			...
			link.l1 = "Это мы ещё посмотрим! Прошу тебя, Господи, направь мою руку, чтобы я смог"+GetSexPhrase("","ла")+" поразить этого нечистого во славу твою!";
			link.l1.go = "skel_fight";
		break;
		...
		break;
		case "meeting_6":
			dialog.text = "Мой собственный старпом! Я считал его своим лучшим другом, даже выдал свою дочь замуж за его сына, а он меня предал, ещё и команду подговорил. Все из-за этого проклятого черепа, будь он не ладен.";
