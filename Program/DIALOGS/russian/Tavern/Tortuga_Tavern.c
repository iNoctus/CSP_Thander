...
//===================================== Голл. линейка, квест №3 =========================================
 	 	case "Step_H3_1":
			dialog.text = NPCStringReactionRepeat("Знаю, как не знать. Он снял у меня комнату и заплатил за год вперёд!", "Хей, не ты ли уже "+ GetSexPhrase("спрашивал","спрашивала") +" меня об этом?",
...
//============================ Голл. линейка, квест №7. узнаем о ожидающем ГГ челе =======================
 	 	case "Step_H7_1":
			dialog.text = NPCStringReactionRepeat("В курсе. Пройди в комнату наверху, там тебя ждёт какой-то тип.", "Ты уже "+ GetSexPhrase("задавал","задавала") +" мне этот вопрос сегодня.", "Опять?", "Может ты не будешь меня отвлекать от работы?", "block", 1, npchar, Dialog.CurrentNode);
        ...
        break;
  	 	case "Step_H7_3":
			dialog.text = "Появился он здесь позавчера, сразу сказал, что ждёт одного человека. Интересовался кораблями из Кюрасао. Выходит, тебя и ждал. А куда он ушёл - мне неведомо.";
...
//============================ Голл. линейка, квест №8. узнаем о Лоране де Графе =======================
  	 	case "Step_H8_1":
			dialog.text = "На Тортуге ты сейчас его не найдёшь. Пару недель назад он ушёл из города. Опять, небось, испанцев потрошить...";
			link.l1 = "А куда он ушёл, точно ты не знаешь?";
        ...
        break;
   	 	case "Step_H8_4":
			dialog.text = "Ладно, скажу тебе, раз так надо. Слышал я, что он курсирует рядом с Картахеной. Де Граф имеет в своём распоряжении фрегат 'Коминте'.";
...
//============================ Исп. линейка, квест №3. узнаем о Роке Бразильце =======================
  	 	case "Step_S3_1":
			...
			link.l1 = "Он мне нужен срочно, у меня к нему серьёзное дело.";
			link.l1.go = "Step_S3_2";
        break;
  	 	case "Step_S3_2":
			dialog.text = "Тогда это проблема. Рок ушёл в поиск к Маракайбо на своём корвете, будет курсировать рядом, пока не подловит галеон с приличной добычей.";
...
//============================ Испанская линейка, квест №4. узнаем о донне Анне. =======================
        ...
        break;
  	 	case "Step_S4_4":
			dialog.text = "Не-е-ет, здесь обоюдное чувство. Донна Анна так же любит нашего генерал-губернатора, как и он её. Слышал я, что они познакомились ещё в Старом Свете. Привёз её на Тортугу один из доверенных офицеров генерал-губернатора - Анри д'Эстре, а где он её нашёл - никто не знает.";
